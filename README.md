...
응 일단 수정작업해야할 파일이 있어.\\
@src\app\(shop)\products\page.tsx 전체상품이 나오는 페이지야. 문제가 몇개 있다.\
 \

1. xx개의 상품을 찾았습니다(전체 상품 개수가 아닌 현재 페이지에 나타나있는 상품의 개수만 나오고있어. ex) 전체상품은  
   100개라고 치면 12개씩 더보기 버튼으로 추가로 나오게 되는데 전체상품개수가 아닌 개수가 나온다. 그래서 infinite scroll로  
   구현하는게 나을거 같아.\
2. 카테고리 항목에 실제 카테고리 이름이 아니고, 개수 또한 실제 개수가 아닌 임시로 코딩된 개수가 적혀있다.
   전자기기(152), 패션(234) 등. 가격 filter 밑에도 이전에 없앤 브랜드 필터가 있어. 브랜드 필터도 삭제해줘.\
3. 그리고 header에 카테고리 / 신상품 / 베스트 이렇게 되어있는데 카테고리 / 전체 상품 / 신상품/ 베스트 로 전체 상품  
   navigation도 추가해줘
